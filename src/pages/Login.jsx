import { useAuth } from '../lib/AuthContext'
import './Login.css'

export default function Login({ accessDenied, denialReason }) {
  const { login } = useAuth()

  return (
    <div className="login-screen">
      <div className="login-grid" aria-hidden="true" />
      <div className="login-card">
        <div className="login-mark">
          <span className="login-mark-dot" />
          SECURIQ <span className="login-mark-sub">| Deadline Tracker</span>
        </div>
        <h1>Deadline tracking, locked down.</h1>
        <p className="login-sub">
          Internal tracker for your team's milestones. No noise, no missed dates.
        </p>

        {accessDenied && denialReason === 'not_allowed' && (
          <div className="login-denied">
            <strong>Access blocked.</strong> Your Google account isn't authorized for this tracker. Ask your admin to add your email before you can sign in.
          </div>
        )}

        {accessDenied && denialReason === 'check_failed' && (
          <div className="login-denied login-denied--warn">
            <strong>Couldn't verify access.</strong> Something interrupted the authorization check (network issue or a browser extension). Please try again.
          </div>
        )}

        {accessDenied && denialReason === 'redirect_failed' && (
          <div className="login-denied login-denied--warn">
            <strong>Sign-in didn't complete.</strong> If you're using the Android app, make sure it opens external/auth links in a real browser (Chrome Custom Tabs), not an embedded webview — Google blocks sign-in from those. Try again, or open this tracker's URL directly in Chrome.
          </div>
        )}

        <button className="login-btn" onClick={login}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          Sign in with Google
        </button>
        <div className="login-footnote mono">restricted to team members · no public access</div>
      </div>
    </div>
  )
}

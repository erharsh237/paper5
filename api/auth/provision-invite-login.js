...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return res.status(200).json({ success: false, error: 'Supabase configuration missing' })
    }
    ...
    if (!inviteMatch) {
      return res.status(200).json({ success: false, error: 'No invitation found for this email address' })
    }

    if (inviteMatch.password_hint && inviteMatch.password_hint !== password) {
      return res.status(200).json({ success: false, error: 'Invalid temporary password' })
    }
    ...
    console.error('[provision-invite-login] Unhandled exception:', err)
